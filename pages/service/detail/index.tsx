...
const ServiceDetail = () => {
  ...
  const features: FeatureCardProps[] = [
    ...
    {
      title: 'Ultra-rapide',
      description: 'Une application à grande vitesse ne décevra pas les clients potentiels.',
      icon: <FiZap />,
    },
  ]
  ...
  return (
    <PageTemplate title="Service Detail - EcoMind">
      ...
      <section className="grid place-items-center gap-16">
        <div className="sm:w-10/12 md:w-8/12 lg:w-6/12 text-center" data-aos="zoom-in-up">
          <PageSentence badge="FONCTIONNALITÉS" title="Voici ce que vous obtiendrez en achetant ce service." />
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {features.map((feature) => (
            <div key={randomString(64)} data-aos="fade-up">
              <FeatureCard title={feature.title} description={feature.description} icon={feature.icon} isNaked />
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="grid place-items-center gap-16">
        <div className="sm:w-10/12 md:w-8/12 lg:w-6/12 text-center" data-aos="zoom-in-up">
          <PageSentence badge="FAQ" title="Questions fréquemment posées, peut-être les mêmes que les vôtres." />
        </div>
